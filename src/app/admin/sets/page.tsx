...
type Word = {
  ...
  wtype?: string | null;
  ipa?: string | null;
  /** Optional, admin-curated depth metadata (§5–6). */
...
export default function AdminSetsPage() {
  ...
  async function exportSet(setId: number, format: "xlsx" | "pdf") {
    ...
    try {
      ...
      // Vocabulary depth travels with the set payload; exports stay relational so a
      // word is never exploded into duplicate rows on the main sheet.
      const depth: Record<string, {
        ...
        topics?: string[];
        families?: Array<{ label: string }>;
      }> = data.content || {};
      const depthOf = (wordId: number) => depth[String(wordId)] || {};
      const collocationRows = current.words.flatMap((word) => (depthOf(word.id).collocations || []).map((item) => ({
        wordId: word.id, term: word.term || "", phrase: item.phrase, meaning: item.meaning || "", example: item.example || "", register: item.register || "",
      })));
      const patternRows = current.words.flatMap((word) => (depthOf(word.id).patterns || []).map((item) => ({
        wordId: word.id, term: word.term || "", pattern: item.pattern, meaning: item.meaning || "", example: item.example || "",
      })));
      const topicRows = current.words.flatMap((word) => (depthOf(word.id).topics || []).map((topic) => ({
        wordId: word.id, term: word.term || "", topic,
      })));
      const familyRows = current.words.flatMap((word) => (depthOf(word.id).families || []).map((family) => ({
        wordId: word.id, term: word.term || "", family: family.label,
      })));
      const depthContent = collocationRows.length || patternRows.length ? [
        { text: "Collocation & cấu trúc", fontSize: 12, bold: true, margin: [0, 14, 0, 6] },
        { table: { headerRows: 1, widths: [110, "*", "*"], body: [
          ["Từ", "Collocation", "Cấu trúc"],
          ...current.words
            .map((word) => [
              word.term || "",
              (depthOf(word.id).collocations || []).map((item) => item.phrase).join("\n"),
              (depthOf(word.id).patterns || []).map((item) => item.pattern).join("\n"),
            ])
            .filter(([, collocations, patterns]) => collocations || patterns),
        ] }, layout: { fillColor: (rowIndex: number) => rowIndex === 0 ? "#EFECFF" : null, hLineColor: () => "#DCD9E8", vLineColor: () => "#DCD9E8" } },
      ] : [];
      ...
      if (format === "xlsx") {
        ...
        // Optional relational sheets; re-import links them back by `term`.
        if (collocationRows.length) XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(collocationRows), "Collocations");
        if (patternRows.length) XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(patternRows), "Patterns");
        if (topicRows.length) XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(topicRows), "Topics");
        if (familyRows.length) XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(familyRows), "WordFamilies");
        ...
        ...depthContent], defaultStyle: { font: "Roboto", fontSize: 8, color: "#242337" }, footer: (page: number, pages: number) => ({ text: `${page}/${pages}`, alignment: "center", fontSize: 8, color: "#8B899F" }) }).download(`${safeName}.pdf`);
      }
          ...
          {editingWordId !== null && (
            <Modal title="Sửa từ" onClose={cancelEditWord} closeOnBackdrop={false}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {detail?.type !== "irregular_verb" && editingWordId !== null && (
                  <WordDepthEditor
                    wordId={editingWordId}
                    term={editForm.term}
                    word={detail.words.find((item) => item.id === editingWordId)}
                    onSaved={() => { if (detail) void openDetail(detail.id, editingWordId ?? undefined); }}
                  />
                )}
