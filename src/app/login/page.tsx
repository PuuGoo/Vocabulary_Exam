      ...
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1180px] overflow-hidden rounded-[24px] border border-line bg-white shadow-[0_20px_70px_rgba(43,39,74,0.08)] lg:grid-cols-[1.05fr_0.95fr]">
        <section className="relative hidden overflow-hidden bg-[#302A68] p-7 text-white lg:block lg:p-14">
          <Link href="/" className="relative z-10 flex items-center gap-3">
            ...
            <span>
              <b className="block text-lg">Lexora</b>
              <span className="block text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-[#C8C2FF]">
                Học viện IELTS
              </span>
            </span>
          </Link>
          <div className="relative z-10 mt-20 max-w-md lg:mt-28">
            ...
            <h1 className="mt-4 text-[clamp(2rem,5vw,3.7rem)] font-extrabold leading-[1.05] tracking-[-0.06em]">
              Tận dụng từng buổi học.
            </h1>
            <p className="mt-6 max-w-sm text-sm leading-6 text-[#D3D0EC]">
              Cách học IELTS bình tĩnh, rõ ràng để xây dựng kỹ năng và sự tự
              tin.
            </p>
            <div className="mt-10 space-y-3 text-sm font-semibold text-[#E6E3FA]">
              <div>✓ Trọng tâm mỗi ngày theo năng lực</div>
              <div>✓ Lặp lại ngắt quãng để nhớ lâu</div>
              <div>✓ Theo dõi tiến bộ rõ ràng</div>
            </div>
          </div>
        </section>
        <section className="flex items-center p-7 sm:p-12 lg:p-16">
          <div className="w-full max-w-[380px]">
            <Link href="/" className="mb-7 flex items-center gap-3 lg:hidden">
              <span className="flex h-10 w-10 items-center justify-center rounded-[13px] bg-gold text-lg font-extrabold text-white">L</span>
              <span><b className="block text-base">Lexora</b><span className="text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-muted">IELTS Academy</span></span>
            </Link>
            <p className="text-sm font-semibold text-gold">Chào mừng trở lại</p>
            <h2 className="mt-2 text-3xl font-extrabold tracking-[-0.05em]">
              Đăng nhập Lexora
            </h2>
            <p className="mt-3 text-sm leading-6 text-muted">
              Tiếp tục nơi bạn đã dừng lại và duy trì chuỗi học tập.
            </p>
            <form onSubmit={handleSubmit} className="mt-8 space-y-4">
              {error && (
                <div className="rounded-[11px] border border-bad/20 bg-badbg px-3 py-2.5 text-sm text-bad">
                  {error}
                </div>
              )}
            ...
            <div className="mt-5 text-center text-xs text-muted">
              <Link
                ...
                className="font-bold text-golddark hover:underline"
              >
                Quên mật khẩu?
              </Link>
            </div>
