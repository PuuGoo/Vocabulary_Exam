  ...
  return <div className="lexora-page-enter space-y-6">
    ...
    <section className="rounded-[22px] bg-[#302A68] p-6 text-white shadow-[0_12px_30px_rgba(48,42,104,0.18)] sm:p-8 [background:linear-gradient(145deg,#302A68_0%,#261f5c_50%,#302A68_100%)] lexora-glow-border">
      ...
      {!review ? <p className="mt-4 text-sm text-[#D3D0EC]">Đang tổng hợp kế hoạch…</p> : review.today.plannedWords === 0 ? <><h2 className="mt-3 text-2xl font-extrabold">✓ Bạn đã hoàn thành phần ôn hôm nay</h2><Link href="/study" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-white px-5 font-bold text-[#302A68]">Học bài mới</Link></> : <>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 text-[0.82rem] sm:grid-cols-4"><span><b className="block text-xl">{review.today.overdueWords}</b><small className="text-[#D3D0EC]">từ quá hạn</small></span><span><b className="block text-xl">{review.today.dueSetReviews}</b><small className="text-[#D3D0EC]">bài cần củng cố</small></span><span><b className="block text-xl">~{review.today.estimatedMinutes}</b><small className="text-[#D3D0EC]">phút</small></span><span><b className="block text-xl">{review.backlog.words}</b><small className="text-[#D3D0EC]">từ backlog</small></span></div>
