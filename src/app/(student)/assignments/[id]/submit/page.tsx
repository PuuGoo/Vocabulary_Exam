...
export default function SubmitAssignmentPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    try {
      ...
      setData(result); setText(result.submission?.textContent || ""); setFile(null); setRemoveFile(false);
  ...
  return <div className={cx.panel}>
    ...
    <form onSubmit={submit}>
      ...
      <div className="mb-4 rounded-lg border border-dashed border-line p-4"><label><span className={cx.label}>Ảnh hoặc tệp đính kèm</span><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain,.doc,.docx" onChange={(event) => { setFile(event.target.files?.[0] || null); setRemoveFile(false); }} className="block w-full text-sm" /></label><p className="mt-2 text-[0.72rem] text-muted">JPG, PNG, WebP, PDF, TXT hoặc Word · tối đa 5 MB.</p>
