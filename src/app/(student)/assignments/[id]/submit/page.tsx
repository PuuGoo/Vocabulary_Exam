...
export default function SubmitAssignmentPage({ params }: { params: { id: string } }) {
  ...
  const draftKey = `lexora_assignment_draft_${params.id}`;
  ...
  async function load() {
    ...
    try {
      ...
      if (!savedText) {
        ...
        if (draft) { setText(draft); setDraftSaved(true); }
      } else {
  ...
  useEffect(() => {
    if (!data || text === initialTextRef.current) return;
  ...
  return <div className={cx.panel}>
    ...
    <form onSubmit={submit}>
      <label><span className={cx.label}>Nội dung văn bản</span><textarea className={cx.input} rows={9} maxLength={10000} value={text} onChange={(event) => setText(event.target.value)} placeholder="Nhập câu trả lời, bài viết hoặc ghi chú cho giáo viên..." /><div className="-mt-2 mb-3 flex items-center justify-between gap-2 text-[0.7rem] text-muted"><span>{draftSaved ? "✓ Đã lưu nháp trên máy" : "Nháp sẽ tự lưu khi bạn nhập"}</span><span>{text.length}/10.000 ký tự</span></div></label>
