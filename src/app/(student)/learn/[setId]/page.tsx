...
export default function LearnPage() {
  ...
  if (!word || finished) return <main className="fixed inset-0 z-[80] overflow-y-auto bg-[#F8F8FC] p-4 pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(24px,env(safe-area-inset-top))] text-[#242337]">
    ...
    {menuOpen && <div ref={menuRef} role="menu" aria-label="Thao tác sau khi hoàn thành" className="flashcard-dock-panel fixed right-4 top-[max(68px,calc(env(safe-area-inset-top)+52px))] z-[90] w-[min(21rem,calc(100vw-2rem))] rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl"><div className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-[#8B899F]">Tiếp tục học</div><div className="grid gap-1"><button data-menu-item role="menuitem" className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => chooseMode("all")}>Học lại tất cả <Shortcut>A</Shortcut></button><button data-menu-item role="menuitem" disabled={unknown === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("unknown")}>Ôn từ chưa nhớ ({unknown}) <Shortcut>U</Shortcut></button><button data-menu-item role="menuitem" disabled={mastered === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("known")}>Ôn lại từ đã nhớ ({mastered}) <Shortcut>K</Shortcut></button><button data-menu-item role="menuitem" disabled={unrated === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("unrated")}>Đánh giá từ chưa thuộc ({unrated}) <Shortcut>N</Shortcut></button><button data-menu-item role="menuitem" className="mt-1 min-h-10 rounded-lg border-t border-[#EBEAF2] px-3 py-2 text-left text-sm text-[#7A4350] hover:bg-[#FFF4F5]" onClick={() => router.push("/study")}>Thoát về danh sách bộ từ <Shortcut>X</Shortcut></button></div></div>}
