...
export default function LearnPage() {
  ...
  useEffect(() => {
    function key(e: KeyboardEvent) {
      ...
      if (menuOpen) {
        ...
        else if (pressed === "l") { e.preventDefault(); router.push(`/listen/${set?.id}`); }
        else if (pressed === "p") { e.preventDefault(); router.push(`/pronunciation/${set?.id}`); }
        else if (pressed === "c" && !isVerb) { e.preventDefault(); router.push(`/sentence/${set?.id}`); }
  ...
  if (!word || finished) return <main className="fixed inset-0 z-[80] overflow-y-auto bg-[#F8F8FC] p-4 pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(24px,env(safe-area-inset-top))] text-[#242337]">
    ...
    {menuOpen && <div ref={menuRef} role="menu" aria-label="Thao tác sau khi hoàn thành" className="flashcard-dock-panel fixed right-4 top-[max(68px,calc(env(safe-area-inset-top)+52px))] z-[90] max-h-[calc(100dvh-90px)] w-[min(21rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl">
      ...
      <div className="grid gap-1">
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/quiz/${set.id}?mode=fill`)}>Điền từ tiếng Anh <Shortcut>F</Shortcut></button>
        {!isVerb && <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/quiz/${set.id}?mode=mc`)}>Trắc nghiệm <Shortcut>Q</Shortcut></button>}
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/dictation/${set.id}`)}>Nghe và viết <Shortcut>D</Shortcut></button>
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/match/${set.id}`)}>Ghép cặp <Shortcut>G</Shortcut></button>
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/listen/${set.id}`)}>Nghe rảnh tay <Shortcut>L</Shortcut></button>
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/pronunciation/${set.id}`)}>Luyện phát âm <Shortcut>P</Shortcut></button>
        {!isVerb && <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/sentence/${set.id}`)}>Xếp câu <Shortcut>C</Shortcut></button>}
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => router.push(`/quiz/${set.id}?mode=fill&timed=1&minutes=15`)}>Thi thử tính giờ <Shortcut>T</Shortcut></button>
        <button data-menu-item role="menuitem" className="mt-1 flex min-h-10 items-center justify-between rounded-lg border-t border-[#EBEAF2] px-3 py-2 text-left text-sm text-[#7A4350] hover:bg-[#FFF4F5]" onClick={() => router.push("/study")}>Thoát về danh sách bộ từ <Shortcut>X</Shortcut></button>
      </div>
    </div>}
    ...
    <header className="relative shrink-0 border-b border-[#EBEAF2] bg-white/95 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] backdrop-blur">
      ...
      {menuOpen && <div ref={menuRef} id="flashcard-quick-menu" role="menu" aria-label="Quick Actions" className="flashcard-dock-panel max-h-[calc(100dvh-90px)] w-[min(21rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl">
        ...
        <div className="grid gap-1">
          ...
          <button data-menu-item role="menuitem" aria-keyshortcuts="L" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>router.push(`/listen/${set.id}`)}><ActionLabel icon="◖">Nghe rảnh tay</ActionLabel><Shortcut>L</Shortcut></button>
          <button data-menu-item role="menuitem" aria-keyshortcuts="P" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>router.push(`/pronunciation/${set.id}`)}><ActionLabel icon="◉">Luyện phát âm</ActionLabel><Shortcut>P</Shortcut></button>
          {!isVerb && <button data-menu-item role="menuitem" aria-keyshortcuts="C" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>router.push(`/sentence/${set.id}`)}><ActionLabel icon="≡">Xếp câu</ActionLabel><Shortcut>C</Shortcut></button>}
