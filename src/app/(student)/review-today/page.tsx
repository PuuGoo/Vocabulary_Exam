...
import { reviewModeHref } from "@/lib/reviewModePolicy";
...
type Word = { id: number; setId: number; setName: string; setType?: string; meaning?: string; term?: string | null; v1?: string | null; v2?: string | null; v3?: string | null; example?: string | null; reasons: string[];
  recommendation?: { mode: string | null; skill: string | null; reason: string; label: string };
  skills?: Array<{ skill: string; mastery: number | null; reason: string }>;
 };
...
export default function ReviewTodayPage() {
  ...
  const [answers, setAnswers] = useState<Array<{ wordId: number; correct: boolean; skill: string; assisted: boolean }>>([]);
  ...
  async function answer(correct: boolean) {
    ...
    // Self-rating is honest but weak evidence: it feeds meaning_recognition as an assisted attempt.
    const nextAnswers = [...answers, { wordId: word.id, correct, skill: "meaning_recognition", assisted: true }];
    ...
    try {
      const response = await fetch("/api/review/plan", {
        ...
        body: JSON.stringify({ idempotencyKey: keyRef.current, setId: batch.setId, expectedStage: batch.reviewRound, mode: "review_today", outcomes: nextAnswers }),
      });
      if (!response.ok) throw new Error();
  ...
  if (started && batch && word) return <div className="mx-auto flex min-h-[calc(100dvh-9rem)] max-w-xl flex-col">
    ...
    {word.recommendation ? (
      <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
        ...
        {reviewModeHref(word.recommendation.mode, word.setId) ? (
          <Link href={reviewModeHref(word.recommendation.mode, word.setId)!} className="inline-flex min-h-9 items-center rounded-full border border-line bg-white px-3 font-bold text-ink">Luyện riêng kỹ năng này →</Link>
        ) : null}
      </div>
    ) : null}
