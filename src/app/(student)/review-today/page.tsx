...
type Word = { id: number; setId: number; setName: string; setType?: string; meaning?: string; term?: string | null; v1?: string | null; v2?: string | null; v3?: string | null; example?: string | null; reasons: string[]; weakSkill?:LearningSkill|null; weakSkillScore?:number|null; recommendedMode?:string|null };
...
export default function ReviewTodayPage() {
  ...
  const weakSkillCounts=useMemo(()=>{const counts=new Map<LearningSkill,number>();for(const item of plan?.batches.flatMap(batch=>batch.words)||[])if(item.weakSkill)counts.set(item.weakSkill,(counts.get(item.weakSkill)||0)+1);return [...counts.entries()].sort((a,b)=>b[1]-a[1]);},[plan]);
  ...
  return <div className="mx-auto max-w-2xl space-y-5">
    ...
    {plan.today.plannedWords === 0 ? <section className="lexora-card p-7 text-center"><div className="text-3xl">✓</div><h2 className="mt-3 text-xl font-extrabold">Bạn đã hoàn thành phần ôn hôm nay</h2>{plan.backlog.words > 0 ? <><p className="mt-2 text-sm text-muted">Còn {plan.backlog.words} từ đến hạn trong backlog.</p><button className="mt-5 min-h-12 rounded-xl bg-[#6550DB] px-6 font-bold text-white" onClick={() => void load(true)}>Ôn thêm</button></> : <Link href="/study" className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-[#6550DB] px-6 font-bold text-white">Học bài mới</Link>}</section> : <>
      ...
      {weakSkillCounts.length>0&&<section className="lexora-card p-5"><h2 className="font-extrabold">Điểm yếu được ưu tiên</h2><div className="mt-3 flex flex-wrap gap-2">{weakSkillCounts.map(([skill,count])=><span key={skill} className="rounded-full bg-[#FFF0E8] px-3 py-2 text-xs font-bold text-[#B75D3B]">{LEARNING_SKILL_LABELS[skill]} · {count}</span>)}</div><p className="mt-3 text-xs text-muted">Lexora vẫn ưu tiên lịch đến hạn và lỗi lặp lại trước, sau đó chọn kỹ năng yếu nhất của từng từ.</p></section>}
