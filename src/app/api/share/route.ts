...
export async function GET(request: NextRequest) {
  ...
  const scoped = await requireAdminResourceAccess({ permission: "sharing.view", folderId: await getShareTargetFolderId(targetType, targetId), level: "viewer", access }); if (isAuthorizationError(scoped)) return scoped;
  try {
...
export async function POST(request: NextRequest) {
  ...
  const scoped = await requireAdminResourceAccess({ permission: "sharing.manage", folderId: await getShareTargetFolderId(parsed.data.targetType, parsed.data.targetId), level: "manager", access }); if (isAuthorizationError(scoped)) return scoped;
