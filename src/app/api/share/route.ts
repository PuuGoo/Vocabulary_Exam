...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ share: share ? { id: share.id, accessMode: share.accessMode, allowedModes: share.allowedModesList, contentSelection: share.contentSelectionList, includeNewContent: share.includeNewContent, customSlug: share.customSlug, publicUrl: getPublicShareUrl({ customSlug: share.customSlug }, request.nextUrl.origin) } : null, content: content ? { sets: content.sets, documents: content.documents, counts: content.counts } : null });
  } catch (error) {
