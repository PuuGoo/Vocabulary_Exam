...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [extension] = await db.insert(assignmentExtensions).values({ assignmentId, userId: parsed.data.userId, dueAt, createdBy: access.userId })
    .onConflictDoUpdate({ target: [assignmentExtensions.assignmentId, assignmentExtensions.userId], set: { dueAt, updatedAt: new Date(), createdBy: access.userId } }).returning();
