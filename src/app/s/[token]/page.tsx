...
export default async function SharedPage({ params, searchParams }: { params: { token: string }; searchParams: { mode?: string; target?: string; set?: string; collection?: string; folder?: string } }) {
  ...
  return <ShareGuestExperience token={params.token} initialMode={searchParams.mode || ""} initialTarget={searchParams.target === "pronunciation" ? "pronunciation" : "term"} payload={result.payload as ShareGuestPayload} />;
}
