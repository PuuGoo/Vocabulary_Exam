...
type StudyMode = "learn" | "fill" | "mc" | "match" | "dictation" | "listen" | "pronunciation" | "sentence" | "timed" | "collocation" | "cloze" | "pattern";
const items: { mode: StudyMode; label: string; icon: string; href: (id: number) => string; depth?: boolean }[] = [
  ...
  // Depth modes are opt-in: a page only shows them when the set really has the
  // authored collocations / patterns / cloze-able examples to practise.
  { mode: "collocation", label: "Collocation", icon: "🔗", href: (id) => `/collocation/${id}`, depth: true },
  { mode: "cloze", label: "Điền theo ngữ cảnh", icon: "📝", href: (id) => `/cloze/${id}`, depth: true },
  { mode: "pattern", label: "Luyện cấu trúc", icon: "🧱", href: (id) => `/pattern/${id}`, depth: true },
  { mode: "timed", label: "Thi thử tính giờ", icon: "⏱", href: (id) => `/quiz/${id}?mode=fill&timed=1&minutes=15` },
];
export default function StudyModeNav({ setId, active, isVerb = false, availableModes, onSelectMode }: { setId: number; active: StudyMode; isVerb?: boolean; availableModes?: readonly string[]; onSelectMode?: (mode: string) => void }) {
  const router = useRouter();
  const scrollerRef = useRef<HTMLDivElement>(null);
  const visibleItems = items.filter((item) => {
    if (isVerb && (item.mode === "mc" || item.mode === "sentence")) return false;
    if (item.depth) return Boolean(availableModes?.includes(item.mode));
    return !availableModes || availableModes.includes(item.mode);
  });
