...
export default function LearnExperience({ authenticatedSetId, initialSet, sourceKey, allowedModes, onSelectMode, onExit, onLoginRequest }: LearnExperienceProps) {
  ...
  const [pinyinRevealed,setPinyinRevealed]=useState(false); const [examplePinyinRevealed,setExamplePinyinRevealed]=useState(false); const [exampleMeaningRevealed,setExampleMeaningRevealed]=useState(false);
  ...
  const selectLearningMode = (nextMode: string, target?: "term" | "pronunciation") => {
    ...
    const id = set?.id;
    if (!id) return;
    const hrefs: Record<string, string> = { fill: `/quiz/${id}?mode=fill${target === "pronunciation" ? "&target=pronunciation" : ""}`, mc: `/quiz/${id}?mode=mc`, dictation: `/dictation/${id}`, match: `/match/${id}`, listen: `/listen/${id}`, pronunciation: `/pronunciation/${id}`, sentence: `/sentence/${id}`, tone: `/tone/${id}`, cloze: `/cloze/${id}`, timed: `/quiz/${id}?mode=fill&timed=1&minutes=15` };
  ...
  async function loadSet() {
    if (initialSet) { setSet(initialSet); setOrder(initialSet.words); setLoading(false); setError(false); return; }
  ...
  const isChinese = normalizeLanguageCode(set?.languageCode) === "zh-CN";
  ...
  useEffect(()=>{setPinyinRevealed(false);setExamplePinyinRevealed(false);setExampleMeaningRevealed(false);},[word?.id]);
  useEffect(() => {
    if (!set || !word) return;
    ...
    <header className="relative shrink-0 border-b border-[#EBEAF2] bg-white px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))]">
      ...
      {menuOpen && <div ref={menuRef} id="flashcard-quick-menu" role="menu" aria-label="Quick Actions" className="flashcard-dock-panel max-h-[calc(100dvh-90px)] w-[min(21rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl">
        ...
        <div className="grid gap-1">
          ...
          {isChinese && (!allowedModes || allowedModes.includes("tone")) && <button data-menu-item role="menuitem" className="flex min-h-10 items-center rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={()=>selectLearningMode("tone")}><ActionLabel icon="〽">Thanh điệu</ActionLabel></button>}
          {isChinese && (!allowedModes || allowedModes.includes("cloze")) && <button data-menu-item role="menuitem" className="flex min-h-10 items-center rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={()=>selectLearningMode("cloze")}><ActionLabel icon="文">Điền từ trong câu</ActionLabel></button>}
    ...
    <section className="relative flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-3 py-3 sm:px-20">
      ...
      <div role="button" tabIndex={0} aria-label={flipped ? "Ẩn đáp án" : "Lật thẻ để xem đáp án"} aria-pressed={flipped} onClick={()=>{if(suppressClick.current){suppressClick.current=false;return;}setFlipped(f=>!f);}} onTouchStart={touchStart} onTouchMove={touchMove} onTouchEnd={touchEnd} onTouchCancel={()=>{swipeRef.current=null;setSwipe(0);}} style={{transform:`translateX(${swipe}px) rotate(${swipe/35}deg)`,transition:swipe?"none":"transform 180ms ease-out"}} className="relative h-[min(58vh,540px)] min-h-[270px] w-full max-w-3xl cursor-pointer select-none rounded-3xl outline-none [perspective:1200px] focus-visible:ring-2 focus-visible:ring-[#7865EE] focus-visible:ring-offset-4">
        ...
        <div className={`flashcard-flipper relative h-full w-full ${flipped?"is-flipped":""}`}>
          ...
          <article className="flashcard-face flashcard-back absolute inset-0 flex h-full flex-col items-center justify-center overflow-y-auto rounded-3xl border border-[#7865EE]/20 bg-white p-6 text-center shadow-[0_14px_40px_rgba(36,35,55,0.08)] sm:p-10"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8B899F]">{isVerb?"V1 — V2 — V3":languageConfig.termLabel}</div><div lang={normalizeLanguageCode(set?.languageCode)} className="mt-4 flex flex-wrap items-center justify-center gap-3 font-serif text-3xl font-bold sm:text-5xl">{answer}<SpeakButton text={getSpeakText(word,set||{})} languageCode={set?.languageCode||"en"}/></div>{!isVerb&&forms.secondary&&<div className="mt-2 text-xl text-[#6E6A83]">{forms.secondary}</div>}{isVerb?<VerbIpa ipaV1={word.ipaV1} ipaV2={word.ipaV2} ipaV3={word.ipaV3} className="mt-3 text-lg"/>:pronunciation&&(isChinese&&!pinyinRevealed?<button type="button" className="mt-3 rounded-lg border border-[#CFC7FF] px-3 py-2 text-sm font-bold text-[#6550DB]" onClick={event=>{event.stopPropagation();setPinyinRevealed(true);}}>Hiện Pinyin</button>:<div className="mt-2 text-xl text-[#765FD5]">{pronunciation}</div>)}{!isVerb&&word.wtype&&<div className="mt-2 text-sm text-[#8B899F]">({word.wtype}){word.level?` · ${word.level}`:""}{word.classifier?` · Lượng từ: ${word.classifier}`:""}</div>}{!isVerb&&word.example&&<div className="mt-4 max-w-xl text-sm italic text-[#8B899F]">“{word.example}”{word.examplePronunciation&&(isChinese&&!examplePinyinRevealed?<button type="button" className="mx-auto mt-2 block rounded-lg border px-3 py-1.5 not-italic" onClick={event=>{event.stopPropagation();setExamplePinyinRevealed(true);}}>Hiện Pinyin câu</button>:<span className="mt-1 block not-italic text-[#765FD5]">{word.examplePronunciation}</span>)}{word.exampleMeaning&&(isChinese&&!exampleMeaningRevealed?<button type="button" className="mx-auto mt-2 block rounded-lg border px-3 py-1.5 not-italic" onClick={event=>{event.stopPropagation();setExampleMeaningRevealed(true);}}>Hiện nghĩa câu</button>:<span className="mt-1 block not-italic">{word.exampleMeaning}</span>)}</div>}</article>
        </div>
      </div>
