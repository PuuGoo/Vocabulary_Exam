...
import WordDepthPanel, { type WordDepthContent } from "@/components/WordDepthPanel";
...
export type LearnWord = {
  ...
  /** Depth content shared with guests; authenticated sets load it from the API. */
  collocations?: Array<{ phrase: string; meaning?: string | null; example?: string | null; register?: string | null }>;
  patterns?: Array<{ pattern: string; meaning?: string | null; example?: string | null }>;
  topics?: string[];
};
...
type UndoState = {
  token: string; message: string; known: Record<number, boolean>; order: LearnWord[]; index: number; flipped: boolean; finished: boolean;
};
...
export default function LearnExperience({ authenticatedSetId, initialSet, sourceKey, allowedModes, onSelectMode, onExit, onLoginRequest }: LearnExperienceProps) {
  const router = useRouter();
  const guest = Boolean(initialSet && !authenticatedSetId);
  const [set, setSet] = useState<LearnSetData | null>(initialSet || null); const [loading, setLoading] = useState(!initialSet); const [error, setError] = useState(false);
  const [order, setOrder] = useState<LearnWord[]>(initialSet?.words || []); const [index, setIndex] = useState(0); const [flipped, setFlipped] = useState(false);
  ...
  const [depth, setDepth] = useState<Record<string, WordDepthContent>>({}); const [mastery, setMastery] = useState<Record<number, SkillMasteryData>>({});
  ...
  const selectLearningMode = (nextMode: string) => {
    ...
    const id = set?.id;
    if (!id) return;
    const hrefs: Record<string, string> = { fill: `/quiz/${id}?mode=fill`, mc: `/quiz/${id}?mode=mc`, dictation: `/dictation/${id}`, match: `/match/${id}`, listen: `/listen/${id}`, pronunciation: `/pronunciation/${id}`, sentence: `/sentence/${id}`, collocation: `/collocation/${id}`, cloze: `/cloze/${id}`, pattern: `/pattern/${id}`, timed: `/quiz/${id}?mode=fill&timed=1&minutes=15` };
  ...
  async function loadSet() {
    if (initialSet) {
      setSet(initialSet); setOrder(initialSet.words); setLoading(false); setError(false);
      // Shared sets travel with their published depth content inline.
      setDepth(Object.fromEntries(initialSet.words
        .filter((item) => (item.collocations?.length || 0) > 0 || (item.patterns?.length || 0) > 0 || (item.topics?.length || 0) > 0)
        .map((item) => [String(item.id), { collocations: item.collocations || [], patterns: item.patterns || [], topics: item.topics || [] }])));
      return;
    }
    setLoading(true); setError(false);
    try {
      const [a,b,c] = await Promise.all([fetch(`/api/sets/${authenticatedSetId}`), fetch("/api/bookmarks").catch(() => null), fetch("/api/study-sessions").catch(() => null)]);
      if (!a.ok) throw new Error(); const data = await a.json(); if (!data.set) throw new Error();
      const progress = data.progress || {};
      setDepth(data.content || {});
      void fetch(`/api/skill-progress?setId=${data.set.id}`)
        .then((response) => (response.ok ? response.json() : null))
        .then((payload) => {
          const entries = (payload?.words || []) as Array<{ wordId: number; overall: number | null; bySkill: SkillMasteryData["bySkill"] }>;
          if (entries.length) setMastery(Object.fromEntries(entries.map((item) => [item.wordId, { overall: item.overall, bySkill: item.bySkill }])));
        })
        .catch(() => undefined);
      let resumeIndex = 0;
      let savedWordId = 0;
      let serverWordId = 0;
      try { savedWordId = Number(sessionStorage.getItem(`lexora-learn-position-${sessionKey}`) || 0); } catch { /* Session storage is optional. */ }
      if (c?.ok) {
      ...
      let found = savedWordId ? data.set.words.findIndex((x: LearnWord) => x.id === savedWordId) : -1;
      if (found < 0 && serverWordId) found = data.set.words.findIndex((x: LearnWord) => x.id === serverWordId);
  ...
  useEffect(() => { void loadSet(); }, [authenticatedSetId, initialSet]);
  ...
  const modeLabel: Record<ReviewMode, string> = { all: "Tất cả từ", unknown: "Từ chưa nhớ", known: "Từ đã nhớ", unrated: "Từ chưa đánh giá" };
  const currentRating = word ? known[word.id] : undefined;
  const currentRatingLabel = currentRating === true ? "Đã nhớ" : currentRating === false ? "Chưa nhớ" : "Chưa đánh giá";
  ...
  useEffect(() => {
    ...
    try { sessionStorage.setItem(`lexora-learn-position-${sessionKey}`, String(word.id)); } catch { /* Session storage is optional. */ }
    if (guest) return;
  ...
  const completeInitialLearning = () => {
    if (guest || !set || mode !== "all" || visitedInThisRun.current.size < set.words.length || initialCompletionSent.current) return;
  ...
  async function mark(learned: boolean) {
    ...
    if (guest) {
      setUndo({ token: "guest", message: learned ? "Đã đánh dấu Đã nhớ" : "Đã đánh dấu Chưa nhớ", ...previous });
  ...
  async function undoLast() {
    ...
    if (guest) { setKnown(current.known); setOrder(current.order); setIndex(current.index); setFlipped(current.flipped); setFinished(current.finished); return; }
  ...
  async function toggleBookmark() {
    ...
    if (guest) { setSaving(false); onLoginRequest?.(); return; }
    try {
  ...
  useEffect(() => {
    function key(e: KeyboardEvent) {
      ...
      if (menuOpen) {
        ...
        else if (pressed === "f") { e.preventDefault(); selectLearningMode("fill"); }
        else if (pressed === "q" && !isVerb) { e.preventDefault(); selectLearningMode("mc"); }
        else if (pressed === "d") { e.preventDefault(); selectLearningMode("dictation"); }
        else if (pressed === "g") { e.preventDefault(); selectLearningMode("match"); }
        else if (pressed === "l") { e.preventDefault(); selectLearningMode("listen"); }
        else if (pressed === "p") { e.preventDefault(); selectLearningMode("pronunciation"); }
        else if (pressed === "c" && !isVerb) { e.preventDefault(); selectLearningMode("sentence"); }
        else if (pressed === "t") { e.preventDefault(); selectLearningMode("timed"); }
        else if (pressed === "x") { e.preventDefault(); exit(); }
        return;
      }
  ...
  if (!word || finished) return <main className="fixed inset-0 z-[80] overflow-y-auto bg-[#F8F8FC] p-4 pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(24px,env(safe-area-inset-top))] text-[#242337]">
    <button ref={menuButtonRef} type="button" aria-label="Mở menu thao tác nhanh" aria-haspopup="menu" aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)} className="flashcard-dock-trigger fixed right-4 top-[max(16px,env(safe-area-inset-top))] z-[91]"><span aria-hidden="true">⚡</span><span className="hidden sm:inline"><Shortcut>M</Shortcut></span></button>
    {menuOpen && <button type="button" aria-label="Đóng menu tùy chọn" className="fixed inset-0 z-[89] cursor-default" onClick={() => setMenuOpen(false)} />}
    {menuOpen && <div ref={menuRef} role="menu" aria-label="Thao tác sau khi hoàn thành" className="flashcard-dock-panel fixed right-4 top-[max(68px,calc(env(safe-area-inset-top)+52px))] z-[90] max-h-[calc(100dvh-90px)] w-[min(21rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl">
      <div className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-[#8B899F]">Tiếp tục học</div>
      <div className="grid gap-1">
        <button data-menu-item role="menuitem" className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => chooseMode("all")}>Học lại tất cả <Shortcut>A</Shortcut></button>
        <button data-menu-item role="menuitem" disabled={unknown === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("unknown")}>Ôn từ chưa nhớ ({unknown}) <Shortcut>U</Shortcut></button>
        <button data-menu-item role="menuitem" disabled={mastered === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("known")}>Ôn lại từ đã nhớ ({mastered}) <Shortcut>K</Shortcut></button>
        <button data-menu-item role="menuitem" disabled={unrated === 0} className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] disabled:opacity-40" onClick={() => chooseMode("unrated")}>Đánh giá từ chưa thuộc ({unrated}) <Shortcut>N</Shortcut></button>
      </div>
      ...
      <div className="grid gap-1">
        ...
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => selectLearningMode("listen")}>Nghe rảnh tay <Shortcut>L</Shortcut></button>
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => selectLearningMode("pronunciation")}>Luyện phát âm <Shortcut>P</Shortcut></button>
        {!isVerb && <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => selectLearningMode("sentence")}>Xếp câu <Shortcut>C</Shortcut></button>}
        <button data-menu-item role="menuitem" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF]" onClick={() => selectLearningMode("timed")}>Thi thử tính giờ <Shortcut>T</Shortcut></button>
        <button data-menu-item role="menuitem" className="mt-1 flex min-h-10 items-center justify-between rounded-lg border-t border-[#EBEAF2] px-3 py-2 text-left text-sm text-[#7A4350] hover:bg-[#FFF4F5]" onClick={exit}>Thoát về danh sách bộ từ <Shortcut>X</Shortcut></button>
      </div>
    </div>}
    <div className="mx-auto flex min-h-full max-w-lg items-center justify-center">
      <section className="w-full rounded-3xl border border-[#EBEAF2] bg-white p-5 text-center shadow-[0_18px_50px_rgba(36,35,55,0.08)] sm:p-8">
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          ...
          <button className="rounded-xl border bg-white px-4 py-2 text-sm font-semibold" onClick={exit}>Chọn bộ khác</button>
        </div>
  ...
  return <main className="fixed inset-0 z-[80] flex flex-col overflow-hidden bg-[#F8F8FC] text-[#242337]">
    <header className="relative shrink-0 border-b border-[#EBEAF2] bg-white px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))]">
      ...
      <div role="progressbar" aria-label="Tiến độ lượt học" aria-valuemin={0} aria-valuemax={total} aria-valuenow={index+1} className="mx-auto mt-3 h-1.5 max-w-4xl overflow-hidden rounded-full bg-[#EBEAF2]"><div className="h-full rounded-full bg-[#7865EE] transition-[width] duration-300" style={{width:`${total?((index+1)/total)*100:0}%`}} /></div>
      ...
      {menuOpen && <div ref={menuRef} id="flashcard-quick-menu" role="menu" aria-label="Quick Actions" className="flashcard-dock-panel max-h-[calc(100dvh-90px)] w-[min(21rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#EBEAF2] bg-white p-3 shadow-xl">
        ...
        <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8B899F]">Ôn tập và đánh giá</div>
        <div className="grid gap-1">
        ...
        <div className="grid gap-1">
          ...
          {(!allowedModes || allowedModes.includes("listen")) && <button data-menu-item role="menuitem" aria-keyshortcuts="L" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>selectLearningMode("listen")}><ActionLabel icon="◖">Nghe rảnh tay</ActionLabel><Shortcut>L</Shortcut></button>}
          {(!allowedModes || allowedModes.includes("pronunciation")) && <button data-menu-item role="menuitem" aria-keyshortcuts="P" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>selectLearningMode("pronunciation")}><ActionLabel icon="◉">Luyện phát âm</ActionLabel><Shortcut>P</Shortcut></button>}
          {!isVerb && (!allowedModes || allowedModes.includes("sentence")) && <button data-menu-item role="menuitem" aria-keyshortcuts="C" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>selectLearningMode("sentence")}><ActionLabel icon="≡">Xếp câu</ActionLabel><Shortcut>C</Shortcut></button>}
          {(!allowedModes || allowedModes.includes("timed")) && <button data-menu-item role="menuitem" aria-keyshortcuts="T" className="flex min-h-10 items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#F0EDFF] focus:bg-[#F0EDFF]" onClick={()=>selectLearningMode("timed")}><ActionLabel icon="◷">Thi thử tính giờ</ActionLabel><Shortcut>T</Shortcut></button>}
          <button data-menu-item role="menuitem" aria-keyshortcuts="X" className="mt-1 flex min-h-10 items-center justify-between rounded-lg border-t border-[#EBEAF2] px-3 py-2 text-left text-sm text-[#7A4350] hover:bg-[#FFF4F5] focus:bg-[#FFF4F5]" onClick={exit}><ActionLabel icon="←">Thoát về danh sách bộ từ</ActionLabel><Shortcut>X</Shortcut></button>
        </div>
    ...
    <section className="relative flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-3 py-3 sm:px-20">
      <button type="button" aria-label="Thẻ trước" disabled={index===0} onClick={prev} className="absolute left-4 top-1/2 hidden h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-[#EBEAF2] bg-white text-xl text-[#6550DB] shadow-sm transition hover:-translate-y-[55%] hover:border-[#CFC7FF] hover:shadow-md disabled:cursor-not-allowed disabled:opacity-30 md:flex">←</button>
      <div role="button" tabIndex={0} aria-label={flipped ? "Ẩn đáp án" : "Lật thẻ để xem đáp án"} aria-pressed={flipped} onClick={()=>{if(suppressClick.current){suppressClick.current=false;return;}setFlipped(f=>!f);}} onTouchStart={touchStart} onTouchMove={touchMove} onTouchEnd={touchEnd} onTouchCancel={()=>{swipeRef.current=null;setSwipe(0);}} style={{transform:`translateX(${swipe}px) rotate(${swipe/35}deg)`,transition:swipe?"none":"transform 180ms ease-out"}} className="relative h-[min(58vh,540px)] min-h-[270px] w-full max-w-3xl cursor-pointer select-none rounded-3xl outline-none [perspective:1200px] focus-visible:ring-2 focus-visible:ring-[#7865EE] focus-visible:ring-offset-4">
        <span className={`pointer-events-none absolute right-4 top-4 z-20 rounded-full px-2.5 py-1 text-[0.68rem] font-bold shadow-sm ${currentRatingClass}`}>{currentRatingLabel}</span>
        ...
        <div className={`flashcard-flipper relative h-full w-full ${flipped?"is-flipped":""}`}>
          ...
          <article className="flashcard-face flashcard-back absolute inset-0 flex h-full flex-col items-center justify-center overflow-y-auto rounded-3xl border border-[#7865EE]/20 bg-white p-6 text-center shadow-[0_14px_40px_rgba(36,35,55,0.08)] sm:p-10"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8B899F]">{isVerb?"V1 — V2 — V3":"Từ tiếng Anh"}</div><div className="mt-4 flex flex-wrap items-center justify-center gap-3 font-serif text-3xl font-bold sm:text-5xl">{answer}<SpeakButton text={isVerb?word.v1||"":word.term||""}/></div>{isVerb?<VerbIpa ipaV1={word.ipaV1} ipaV2={word.ipaV2} ipaV3={word.ipaV3} className="mt-3 text-lg"/>:word.ipa&&<div className="mt-2 text-xl text-[#765FD5]">{word.ipa}</div>}{!isVerb&&word.wtype&&<div className="mt-2 text-sm text-[#8B899F]">({word.wtype})</div>}{!isVerb&&word.example&&<div className="mt-4 max-w-xl text-sm italic text-[#8B899F]">“{word.example}”</div>}{!isVerb&&<WordDepthPanel content={depth[String(word.id)]} className="mx-auto" />}</article>
        </div>
      </div>
    ...
    <footer className="shrink-0 border-t border-[#EBEAF2] bg-white px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-3">
      {!isVerb && word && mastery[word.id] && (
        <details className="mx-auto mb-2 max-w-2xl rounded-xl border border-[#EBEAF2] bg-white px-3 py-2">
          <summary className="cursor-pointer text-xs font-bold text-[#6550DB]">Kỹ năng của từ này</summary>
          <SkillMasteryPanel summary={mastery[word.id]} className="mt-2" />
          <p className="mt-2 text-[0.68rem] text-muted">“Đã nhớ” là tự đánh giá. Lexora vẫn ôn những kỹ năng còn yếu như collocation hay phát âm.</p>
        </details>
      )}
