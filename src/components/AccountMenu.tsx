...
function MenuLink({
  ...
  return (
    ...
    >
      <span
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[11px] border border-[#E5E1FF] bg-[#F3F0FF] text-sm font-bold text-[#6550DB] transition group-hover:border-[#D3CBFF]"
        aria-hidden="true"
      >
        {icon}
      </span>
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block truncate text-[0.67rem] text-muted">
          {description}
        </span>
      </span>
      <span
        className="text-sm text-[#B0ADBD] transition group-hover:translate-x-0.5 group-hover:text-gold"
        aria-hidden="true"
      >
        ›
      </span>
    ...
    <div ref={rootRef} className="relative ml-1">
      ...
      >
        <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#ECE9FF] text-xs font-extrabold text-[#6550DB]">
          ...
          <span
            className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-white bg-[#36B37E]"
            aria-label="Đang hoạt động"
          />
        </span>
        <span className="hidden max-w-28 truncate text-left text-[0.72rem] font-bold text-ink xl:block">
          {displayName}
        </span>
        <span
          className={`hidden text-[0.65rem] text-muted transition-transform sm:block ${open ? "rotate-180" : ""}`}
          aria-hidden="true"
        >
          ⌄
        </span>
        ...
        >
          <div className="rounded-[14px] bg-[#F4F1FF] p-3.5">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white text-sm font-extrabold text-[#6550DB] shadow-sm">
                {initials(displayName)}
              </span>
              <span className="min-w-0 flex-1">
              ...
              <span className="rounded-full border border-[#DCD6FF] bg-white px-2 py-1 text-[0.6rem] font-bold uppercase tracking-wide text-[#6550DB]">
                {roleLabel}
              </span>
            </div>
          </div>
          ...
          <div className="mt-1">
            <MenuLink
              ...
              description="Email khôi phục, đổi mật khẩu"
            ...
            {!adminMode && (
              <MenuLink
                ...
                description="Xem kết quả và mục tiêu của bạn"
                onClick={() => setOpen(false)}
              />
            )}
            {isAdmin && (
              <MenuLink
                href={adminMode ? "/dashboard" : "/admin"}
                icon={adminMode ? "A" : "⚙"}
                title={adminMode ? "Chuyển sang học tập" : "Mở khu quản trị"}
                description={
                  adminMode
                    ? "Học với quyền học viên"
                    : "Quản lý nội dung và học viên"
                }
                onClick={() => setOpen(false)}
              />
            )}
