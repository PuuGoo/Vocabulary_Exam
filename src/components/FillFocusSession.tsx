...
export default function FillFocusSession({
  ...
  return (
    <div className={`fill-focus-session lexora-page-enter ${expandOpen ? "fixed inset-0 z-[90] overflow-y-auto bg-[rgb(var(--color-paper))] p-3 sm:p-6" : ""} ${chrome === "compact" ? "space-y-1.5 pb-1.5" : "space-y-1.5 pb-1.5"}`}>
      ...
      <section className={`rounded-xl border border-line bg-white ${chrome === "compact" ? "p-2.5" : "p-2.5 sm:p-3"}`}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="inline-flex rounded-lg bg-[#F4F2FA] p-0.5" aria-label="Mục tiêu phiên học">
            <button className={`min-h-8 rounded-md px-2.5 text-[0.78rem] font-bold ${sessionKind === "practice" ? "bg-white text-ink shadow-sm" : "text-muted"}`} onClick={() => navigateWith({ session: null, view: "focus" })}>Luyện tập</button>
            <button className={`min-h-8 rounded-md px-2.5 text-[0.78rem] font-bold ${sessionKind === "test" ? "bg-white text-ink shadow-sm" : "text-muted"}`} onClick={() => navigateWith({ session: "test", view: "focus" })}>Kiểm tra</button>
          </div>
          <div className="inline-flex rounded-lg border border-line p-0.5" aria-label="Cách hiển thị câu hỏi">
            <button className="min-h-8 rounded-md bg-ink px-2.5 text-[0.78rem] font-bold text-white">Focus</button>
            <button className="min-h-8 rounded-md px-2.5 text-[0.78rem] font-bold text-muted" onClick={() => navigateWith({ view: "list" })}>Xem cả nhóm</button>
          </div>
          <button className="min-h-8 rounded-md px-2 text-xs font-bold text-golddark hover:bg-goldpale/40" onClick={() => setCustomOpen((open) => !open)} aria-expanded={customOpen}>⚙ Tùy chỉnh</button>
        </div>
        <div className="mt-1.5 flex items-center justify-between gap-3 text-sm font-semibold text-muted">
          ...
          <button type="button" aria-haspopup="dialog" aria-expanded={navigatorOpen} onClick={() => setNavigatorOpen(true)} className="min-h-8 rounded-md border border-line bg-white px-2 text-xs font-bold text-ink hover:bg-[#F8F7FF]">{isRetry ? "↻ Ôn lại" : `Câu ${Math.min(cursor + 1, originalWords.length)}/${originalWords.length}`} · Xem câu</button>
      ...
      {phase === "complete" ? (
        ...
        <>
          <section className={`fill-focus-card mx-auto w-full ${expandOpen ? "max-w-[120rem]" : "max-w-4xl"} rounded-xl border border-line bg-white shadow ${chrome === "compact" ? "p-3 sm:p-4" : "p-4"}`} aria-live="polite">
            ...
            {!feedback && currentHint && <div className="mx-auto mt-2 w-full max-w-3xl rounded-lg border border-dashed border-gold bg-goldpale/25 px-3 py-2 text-sm"><div className="text-xs font-bold uppercase tracking-wide text-golddark">{multiGroup ? `Gợi ý cấu trúc ${hintGroupIndex + 1} · ` : ""}{currentHint.label}</div><div className={`mt-1 ${currentHint.revealed ? "font-bold text-ink" : "font-mono text-muted"}`}>{currentHint.value}</div></div>}
