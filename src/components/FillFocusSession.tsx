          ...
          <section className={`fill-focus-card mx-auto w-full ${expandOpen ? "max-w-[120rem]" : "max-w-4xl"} fill-focus-card-inner rounded-2xl border border-line bg-white shadow-sm sm:shadow ${chrome === "compact" ? "p-3 sm:p-4" : "p-3.5 sm:p-5"}`} aria-live="polite">
            ...
            {multiGroup ? (
              ...
              <div className="mx-auto mt-2 w-full max-w-3xl">
                ...
                <input ref={inputRef} id={`fill-answer-${currentWord.id}`} lang={languageCode} type="text" autoComplete="off" autoCapitalize="none" spellCheck={false} readOnly={Boolean(feedback)} className={`fill-answer-input ${cx.input} !mb-0 mt-1.5 !border-2 !border-[#C9C2F5] min-h-14 text-xl font-bold text-center text-ink placeholder:font-normal placeholder:text-muted focus:!border-gold focus:!shadow-[0_0_0_4px_rgba(120,101,238,0.16)] ${feedback ? feedback.correct ? "!border-ok !bg-okbg/40" : "!border-bad !bg-badbg/30" : ""}`} value={currentAnswer} onChange={(event) => { if (!feedback) setAnswers((current) => ({ ...current, [currentWord.id]: event.target.value })); }} onKeyDown={onInputKeyDown} enterKeyHint="done" />
              </div>
            )}

            {feedback && <AnswerFeedback word={currentWord} feedback={feedback} outcome={currentOutcome} set={set} target={target} />}

            {feedback && (needsCorrection || feedback.corrected) && <div className="mx-auto mt-2 w-full max-w-3xl rounded-xl border border-bad/25 bg-badbg/30 p-3"><div className={`text-sm font-bold ${feedback.corrected ? "text-ok" : "text-bad"}`}>{feedback.corrected ? "✓ Đã sửa đúng" : multiGroup ? `Gõ lại ${correction.length} cấu trúc còn thiếu hoặc sai` : `Gõ lại ${target === "pronunciation" ? "Pinyin" : languageConfig.termLabel.toLowerCase()}`}</div>{correction.map((value, correctionIndex) => <div key={correctionIndex} className="mt-2"><label className="text-xs font-semibold text-muted" htmlFor={`fill-correction-${currentWord.id}-${correctionIndex}`}>{multiGroup ? `Cấu trúc cần sửa ${correctionIndex + 1}` : target === "pronunciation" ? "Pinyin" : languageConfig.termLabel}</label><input ref={(element) => { correctionInputRefs.current[correctionIndex] = element; }} id={`fill-correction-${currentWord.id}-${correctionIndex}`} autoFocus={correctionIndex === 0} type="text" autoComplete="off" autoCapitalize="none" spellCheck={false} readOnly={Boolean(feedback.corrected)} className={`${cx.input} !mb-0 mt-1 min-h-14 text-lg`} value={value} onChange={(event) => { if (!feedback.corrected) setCorrection((current) => current.map((item, index) => index === correctionIndex ? event.target.value : item)); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.nativeEvent.isComposing && !event.repeat && correctionIndex < correction.length - 1) { event.preventDefault(); correctionInputRefs.current[correctionIndex + 1]?.focus(); return; } onInputKeyDown(event); }} enterKeyHint={correctionIndex === correction.length - 1 ? "done" : "next"} /></div>)}</div>}

            <div className="fill-session-cta sticky bottom-0 z-10 mx-auto mt-2.5 w-full max-w-2xl bg-gradient-to-t from-white via-white to-transparent pb-1.5 pt-2.5">
              {effectiveSessionKind === "test" ? (cursor === originalWords.length - 1 ? <button className={`${cx.btn} ${cx.btnGold} min-h-[3.25rem] w-full !rounded-[14px] !text-[1rem] shadow-[0_10px_24px_rgba(120,101,238,0.28)] transition hover:!-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(120,101,238,0.34)]`} disabled={!allTestAnswered || saving} onClick={() => void submitTestGroup()}>{saving ? "Đang lưu…" : allTestAnswered ? "Nộp nhóm" : `Còn ${originalWords.length - totalAnsweredInTest} câu chưa làm`}</button> : <button className={`${cx.btn} ${cx.btnGold} min-h-[3.25rem] w-full !rounded-[14px] !text-[1rem] shadow-[0_10px_24px_rgba(120,101,238,0.28)] transition hover:!-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(120,101,238,0.34)]`} disabled={!currentResponseComplete} onClick={advanceTest}>Câu tiếp theo →</button>) : !feedback ? <button className={`${cx.btn} ${cx.btnGold} min-h-[3.25rem] w-full !rounded-[14px] !text-[1rem] shadow-[0_10px_24px_rgba(120,101,238,0.28)] transition hover:!-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(120,101,238,0.34)]`} disabled={!currentResponseComplete} onClick={checkPracticeAnswer}>Kiểm tra</button> : needsCorrection ? <button className={`${cx.btn} ${cx.btnGold} min-h-[3.25rem] w-full !rounded-[14px] !text-[1rem] shadow-[0_10px_24px_rgba(120,101,238,0.28)] transition hover:!-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(120,101,238,0.34)]`} disabled={!correction.length || correction.some((value) => !value.trim())} onClick={confirmCorrection}>Xác nhận sửa</button> : <button className={`${cx.btn} ${cx.btnGold} min-h-[3.25rem] w-full !rounded-[14px] !text-[1rem] shadow-[0_10px_24px_rgba(120,101,238,0.28)] transition hover:!-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(120,101,238,0.34)]`} onClick={advancePractice}>Câu tiếp theo →</button>}
            </div>
          </section>
...
function AnswerFeedback({ word, feedback, outcome, set, target }: { word: FillFocusWord; feedback: Feedback; outcome?: FillRecallOutcome; set: Props["set"]; target: FillTarget }) {
  ...
  if (feedback.corrected) return <div className="mx-auto mt-2 w-full max-w-3xl rounded-xl border border-ok/30 bg-okbg/35 p-4"><div className="font-bold text-ok">✓ Đã sửa đúng</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
  if (feedback.correct) return <div className="mx-auto mt-2 w-full max-w-2xl rounded-xl border border-ok/30 bg-okbg/35 p-3"><div className="font-bold text-ok">✓ {outcome?.firstTryCorrect ? "Chính xác" : "Đúng sau hỗ trợ"}</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
