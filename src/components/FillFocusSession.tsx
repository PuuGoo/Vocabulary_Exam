    ...
    <div className={`fill-focus-session lexora-page-enter ${chrome === "compact" ? "space-y-2 pb-2" : "space-y-3 pb-4"}`}>
      ...
      <section className={`rounded-xl border border-line bg-white ${chrome === "compact" ? "p-2" : "p-2.5 sm:p-3"}`}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <button className="min-h-10 rounded-lg px-2 text-xs font-bold text-golddark hover:bg-goldpale/40" onClick={() => setCustomOpen((open) => !open)} aria-expanded={customOpen}>⚙ Tùy chỉnh</button>
        </div>
        <div className="mt-2 flex items-center justify-between gap-2 text-xs font-semibold text-muted">
          ...
          <button type="button" aria-haspopup="dialog" aria-expanded={navigatorOpen} onClick={() => setNavigatorOpen(true)} className="min-h-9 rounded-lg border border-line bg-white px-2 font-bold text-ink hover:bg-[#F8F7FF]">{isRetry ? "↻ Ôn lại" : `Câu ${Math.min(cursor + 1, originalWords.length)}/${originalWords.length}`} · Xem câu</button>
          <span>{progressPercent}%</span>
        </div>
        <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-line"><div className="h-full rounded-full bg-gold transition-[width]" style={{ width: `${progressPercent}%` }} /></div>
      ...
      {phase === "complete" ? (
        ...
        <>
          <section className={`fill-focus-card mx-auto w-full max-w-4xl rounded-2xl border border-line bg-white shadow-sm ${chrome === "compact" ? "p-3 sm:p-4" : "p-5 sm:p-7"}`} aria-live="polite">
            ...
            {!feedback && currentHint && <div className="mx-auto mt-4 w-full max-w-2xl rounded-xl border border-dashed border-gold bg-goldpale/25 px-4 py-3 text-sm"><div className="text-xs font-bold uppercase tracking-wide text-golddark">{multiGroup ? `Gợi ý cấu trúc ${hintGroupIndex + 1} · ` : ""}{currentHint.label}</div><div className={`mt-1 ${currentHint.revealed ? "font-bold text-ink" : "font-mono text-muted"}`}>{currentHint.value}</div></div>}

            {multiGroup ? (
              <div className="mx-auto mt-5 grid w-full max-w-2xl gap-3.5">
...
function AnswerFeedback({ word, feedback, outcome, set, target }: { word: FillFocusWord; feedback: Feedback; outcome?: FillRecallOutcome; set: Props["set"]; target: FillTarget }) {
  ...
  if (feedback.corrected) return <div className="mx-auto mt-5 w-full max-w-2xl rounded-xl border border-ok/30 bg-okbg/35 p-4"><div className="font-bold text-ok">✓ Đã sửa đúng</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
