...
function AnswerFeedback({ word, feedback, outcome, set, target }: { word: FillFocusWord; feedback: Feedback; outcome?: FillRecallOutcome; set: Props["set"]; target: FillTarget }) {
  ...
  if (feedback.corrected) return <div className="mx-auto mt-3 w-full max-w-3xl rounded-xl border border-ok/30 bg-okbg/35 p-5"><div className="font-bold text-ok">✓ Đã sửa đúng</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
  if (feedback.correct) return <div className="mx-auto mt-3 w-full max-w-2xl rounded-xl border border-ok/30 bg-okbg/35 p-4"><div className="font-bold text-ok">✓ {outcome?.firstTryCorrect ? "Chính xác" : "Đúng sau hỗ trợ"}</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
