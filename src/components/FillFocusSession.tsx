...
export default function FillFocusSession({
  ...
  return (
    <div className={`fill-focus-session lexora-page-enter ${expandOpen ? "fixed inset-0 z-[90] overflow-y-auto bg-[rgb(var(--color-paper))] p-3 sm:p-6" : ""} ${chrome === "compact" ? "space-y-2 pb-2" : "space-y-3 pb-4"}`}>
      ...
      <section className={`rounded-xl border border-line bg-white ${chrome === "compact" ? "p-3" : "p-4 sm:p-5"}`}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="inline-flex rounded-lg bg-[#F4F2FA] p-1" aria-label="Mục tiêu phiên học">
            ...
            <button className={`min-h-12 rounded-lg px-4 text-[0.9rem] font-bold ${sessionKind === "test" ? "bg-white text-ink shadow-sm" : "text-muted"}`} onClick={() => navigateWith({ session: "test", view: "focus" })}>Kiểm tra</button>
          </div>
          <div className="inline-flex rounded-lg border border-line p-1" aria-label="Cách hiển thị câu hỏi">
            <button className="min-h-12 rounded-lg bg-ink px-4 text-[0.9rem] font-bold text-white">Focus</button>
            <button className="min-h-12 rounded-lg px-4 text-[0.9rem] font-bold text-muted" onClick={() => navigateWith({ view: "list" })}>Xem cả nhóm</button>
          </div>
      ...
      {phase === "complete" ? (
        ...
        <>
          <section className={`fill-focus-card mx-auto w-full ${expandOpen ? "max-w-[120rem]" : "max-w-5xl"} rounded-2xl border border-line bg-white shadow-md ${chrome === "compact" ? "p-4 sm:p-5" : "p-6 sm:p-8"}`} aria-live="polite">
            ...
            {!feedback && currentHint && <div className="mx-auto mt-5 w-full max-w-3xl rounded-xl border border-dashed border-gold bg-goldpale/25 px-4 py-3 text-sm"><div className="text-xs font-bold uppercase tracking-wide text-golddark">{multiGroup ? `Gợi ý cấu trúc ${hintGroupIndex + 1} · ` : ""}{currentHint.label}</div><div className={`mt-1 ${currentHint.revealed ? "font-bold text-ink" : "font-mono text-muted"}`}>{currentHint.value}</div></div>}
...
function AnswerFeedback({ word, feedback, outcome, set, target }: { word: FillFocusWord; feedback: Feedback; outcome?: FillRecallOutcome; set: Props["set"]; target: FillTarget }) {
  ...
  if (feedback.corrected) return <div className="mx-auto mt-5 w-full max-w-3xl rounded-xl border border-ok/30 bg-okbg/35 p-5"><div className="font-bold text-ok">✓ Đã sửa đúng</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
  if (feedback.correct) return <div className="mx-auto mt-5 w-full max-w-2xl rounded-xl border border-ok/30 bg-okbg/35 p-4"><div className="font-bold text-ok">✓ {outcome?.firstTryCorrect ? "Chính xác" : "Đúng sau hỗ trợ"}</div>{answerDetails}<div className="mt-2">{speak}</div>{word.example && <div className="mt-2 text-sm italic text-muted">{word.example}</div>}</div>;
