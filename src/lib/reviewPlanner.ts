import { recommendReviewMode, type ReviewRecommendation, type ReviewReasonCode } from "@/lib/reviewModePolicy";
import type { SkillPriority } from "@/lib/wordSkills";
...
export type ReviewWord = {
  ...
  wrongCount: number;
  timesWrong: number;
  /** Optional skill evidence attached by the review service for the shortlist. */
  skills?: SkillPriority[];
  /** Modes that are actually implemented for this word's content. */
...
export type PlannedWord = ReviewWord & {
  ...
  daysOverdue: number;
  reasons: Array<"overdue" | "due" | "difficult" | "forgotten" | "set_review" | "weak_skill" | "unpracticed">;
  /** Which dimension this word should be practised in, and why it is due. */
...
export function rankReviewCandidate(word: ReviewWord, now = new Date(), setReview?: DueSetReview) {
  ...
  const recommendation = recommendReviewMode({
    reasons: reasons as ReviewReasonCode[],
    skillPriorities: word.skills,
    availableModes: word.availableModes,
    fallbackMode: word.availableModes?.includes("fill") ? "fill" : undefined,
  });
  if (recommendation.reason === "weak_skill" && !reasons.includes("weak_skill")) reasons.push("weak_skill");
  if (recommendation.reason === "unpracticed" && !reasons.includes("unpracticed")) reasons.push("unpracticed");
  // Existing scheduling stays authoritative: a weak skill nudges priority below
  // due/mistake signals but above plain staleness.
  const skillBoost = recommendation.reason === "weak_skill" ? 90 : recommendation.reason === "unpracticed" ? 25 : 0;
  const priorityScore = skillBoost +
  ...
  return { ...word, priorityScore, daysOverdue, reasons, recommendation };
}
