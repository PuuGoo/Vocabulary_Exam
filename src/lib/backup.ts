...
export const BACKUP_COLLECTIONS = [
  ...
  "dailyActivities",
  // Vocabulary depth (v4). Older backups simply do not contain these keys.
  "wordCollocations", "wordPatterns", "wordPronunciations",
...
/** Collections that may be absent from older backup versions. */
const OPTIONAL_COLLECTIONS = new Set<string>([
  "vocabCategories", "categoryDocuments", "appSettings", "setReviewProgress", "reviewSessions",
  "wordCollocations", "wordPatterns", "wordPronunciations",
  "topics", "wordTopics", "wordFamilies", "wordFamilyMembers", "wordSkillProgress",
]);
...
export function parseBackupDocument(value: unknown): BackupDocument {
  ...
  for (const collection of BACKUP_COLLECTIONS) {
    ...
    if (OPTIONAL_COLLECTIONS.has(collection) && rows === undefined) {
      data[collection] = [];
      continue;
    }
