...
export type RestoreResult =
  | { kind: "preview"; preview: ReturnType<typeof getBackupCounts>; unknownUsers: string[]; createdAt: string; version: number; integrity: "verified" | "legacy" }
  | { kind: "report"; report: Report }
  | { kind: "error"; error: string };

export async function runRestore(parsed: unknown, action: string, confirmation: string): Promise<RestoreResult> {
  if (parsed == null || typeof parsed !== "object" || Array.isArray(parsed)) {
    return { kind: "error", error: "File JSON không hợp lệ." };
  }
  try {
    const raw = JSON.stringify(parsed);
    if (raw.length > MAX_JSON_CHARS) {
      return { kind: "error", error: `File JSON vượt quá ${(MAX_JSON_CHARS / 1024 / 1024).toFixed(0)} MB.` };
    }
  } catch {
    return { kind: "error", error: "File JSON chứa dữ liệu tuần hoàn không hợp lệ." };
  }
  ...
  if (integrity === "invalid") {
    return { kind: "error", error: "File sao lưu đã bị thay đổi hoặc hư hỏng. Không thể khôi phục an toàn." };
  }
  ...
  if (action === "preview") {
    return {
      kind: "preview",
      preview: getBackupCounts(backup),
      unknownUsers,
  ...
  if (action !== "restore" || confirmation !== CONFIRMATION_WORD) {
    return { kind: "error", error: `Nhập chính xác "${CONFIRMATION_WORD}" để xác nhận.` };
  }
  ...
  await db.transaction(async (tx) => {
    ...
    for (const row of backup.data.vocabCategories) {
      ...
      const [created] = await tx.insert(vocabCategories).values({ name, createdBy: userMap.get(nullableNumber(row, "createdBy") ?? -1) ?? null, createdAt: date(row, "createdAt") }).returning({ id: vocabCategories.id });
      categoriesByName.set(name.toLocaleLowerCase("vi"), created.id); report.added.vocabCategories++;
    }
