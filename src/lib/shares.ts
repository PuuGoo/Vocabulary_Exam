...
import { loadWordContent, type WordContent } from "@/lib/wordContent";
...
export async function getPublicSharePayload(token: string, requestedMode?: string, requestedSetId?: number, requestedCollection?: string, requestedFolder?: string) {
  ...
  if (share.targetType === "vocab_set") {
    ...
    // Shared vocabulary carries its published collocations / patterns / topics.
    // Draft content and anything outside the share scope stays invisible.
    const sharedDepth = await loadWordContent(publicWords.map((word) => word.id));
    const wordsWithDepth = publicWords.map((word) => ({ ...word, ...serializeSharedDepth(sharedDepth.get(word.id)) }));
    return { share, payload: { targetType: share.targetType, title: set.name, count: publicWords.length, setType: set.type, allowedModes: share.allowedModesList, words: wordsWithDepth } };
  }
  ...
  if (requestedSetId) {
    ...
    const sharedDepth = await loadWordContent(publicWords.map((word) => word.id));
    const wordsWithDepth = publicWords.map((word) => ({ ...word, ...serializeSharedDepth(sharedDepth.get(word.id)) }));
    return { share, payload: { targetType: "vocab_set", title: set.name, count: publicWords.length, setType: set.type, allowedModes: share.allowedModesList.filter((mode) => modesForSetType(set.type).includes(mode)), words: wordsWithDepth } };
  }
...
/** Minimal, published-only depth payload for guests (no ids, no drafts). */
function serializeSharedDepth(content: WordContent | undefined) {
  return {
    collocations: (content?.collocations || []).map((item) => ({
      phrase: item.phrase, meaning: item.meaning, example: item.example, register: item.register,
    })),
    patterns: (content?.patterns || []).map((item) => ({
      pattern: item.pattern, meaning: item.meaning, example: item.example,
    })),
