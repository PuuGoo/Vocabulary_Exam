...
test("assignmentProgress ignores attempts before assignment and wrong modes", () => {
  const result = assignmentProgress(assignment, [attempt({ createdAt: new Date("2026-07-19T00:00:00Z") }), attempt({ mode: "fill" })], beforeDue);
  assert.equal(result.status, "pending");
  assert.equal(result.attemptCount, 0);
});

test("assignmentProgress distinguishes in progress, completed, late and overdue", () => {
  assert.equal(assignmentProgress(assignment, [attempt({ score: 6 })], beforeDue).status, "in_progress");
  assert.equal(assignmentProgress(assignment, [attempt()], beforeDue).status, "completed");
  assert.equal(assignmentProgress(assignment, [attempt({ createdAt: new Date("2026-07-23T00:00:00Z") })], beforeDue).status, "completed_late");
